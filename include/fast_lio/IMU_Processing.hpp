                              ...
                              PointCloudXYZI& pcl_out)
{
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); --it_kp)
    {
        ...
        for (; it_pcl->curvature / static_cast<double>(1000) > head->offset_time; --it_pcl)
        {
            ...
            V3D P_compensate = imu_state.offset_R_L_I.conjugate() * (imu_state.rot.conjugate() * (R_i * (imu_state.
                offset_R_L_I * P_i + imu_state.offset_T_L_I) + T_ei) - imu_state.offset_T_L_I); // not accurate!
